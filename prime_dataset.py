...
class FloorplanDatasetPrime(Dataset):
    """
    This is a helper Dataset class that reads the raw FloorSet-Prime data files (.pth).
    It is used internally by the main FloorSetPrimeDataset and assumes the dataset
    is already downloaded and present locally.
    """
    def __init__(self, root, set_name='train'):
        self.root = root
        self.set_name = set_name
        
        # --- EDIT: Changed the path to match the user's directory structure ---
        # The new path searches through all 'config_*' subdirectories.
        data_pattern = os.path.join(self.root, 'config_*', 'primedata_*.pth')
        label_pattern = os.path.join(self.root, 'config_*', 'primelabel_*.pth')
        
        self.data_files = sorted(glob(data_pattern))
        self.label_files = sorted(glob(label_pattern))
        
        if not self.data_files or not self.label_files:
            raise FileNotFoundError(
                f"No data files found matching the pattern in '{self.root}'. "
                f"Please ensure the PrimeTensorData directory contains 'config_*' subfolders with .pth files."
            )

    def __getitem__(self, index):
        """
        Loads a single raw floorplan sample from disk.
        """
        try:
            # --- EDIT: Added weights_only=True to address the FutureWarning ---
            data = torch.load(self.data_files[index], weights_only=True)
            label = torch.load(self.label_files[index], weights_only=True)
        except Exception as e:
            print(f"Error loading file at index {index}: {self.data_files[index]}")
            raise e
        
        # Returns a dictionary containing the raw tensors from the FloorSet dataset.
        sample = {
            'fp_sol': data[0],             # Polygon vertices for each block
            'b2b_connectivity': label[2], # Block-to-block connections
        }
        return sample
...
#         # --- EDIT: Changed the path to match the user's directory structure ---
#         # The original path was os.path.join(self.root, 'Prime', self.set_name)
#         # The new path searches through all 'config_*' subdirectories.
        
#         # Note: This assumes all config_* folders are part of the 'train' set.
#         # If you have a separate test set, you would need to adjust the folder structure.
#         data_pattern = os.path.join(self.root, 'config_*', 'primedata_*.pth')
#         label_pattern = os.path.join(self.root, 'config_*', 'primelabel_*.pth')
...
#             label = torch.load(self.label_files[index])
#         except Exception as e:
#             print(f"Error loading file at index {index}: {self.data_files[index]}")
#             raise e
...
#         # Construct the path to the dataset files
#         data_path = os.path.join(self.root, 'Prime', self.set_name)
        
#         self.data_files = sorted(glob(os.path.join(data_path, 'data', '*.pth')))
#         self.label_files = sorted(glob(os.path.join(data_path, 'label', '*.pth')))
        
#         if not self.data_files or not self.label_files:
#             raise FileNotFoundError(
#                 f"No data files found in '{data_path}'. "
#                 f"Please ensure the FloorSet dataset is correctly placed in the '{root}' directory."
#             )
